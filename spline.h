...
private:
    ...
    double valueAt(double x, int idx) const;
    double derivAt(int order, double x, int idx) const;
...
{
    assert(m_n==0);          // set_points() must not have happened yet
    m_left=left;
    m_right=right;
...
void spline::set_points(int n, const double* x,
                        const double* y, bool cubic_spline)
{
    m_n = n;
    m_x = x;
...
double spline::valueAt(double x, int idx) const
{
    double h=x-m_x[idx];
    double interpol;
    if(x<m_x[0]) {
        ...
        interpol=(m_b[m_n-1]*h + m_c[m_n-1])*h + m_y[m_n-1];
    } else {
        // interpolation
...
double spline::derivAt(int order, double x, int idx) const
{
    assert(order>0);
    ...
    if(x<m_x[0]) {
        ...
        switch(order) {
        case 1:
            interpol=2.0*m_b[m_n-1]*h + m_c[m_n-1];
            break;
        case 2:
            interpol=2.0*m_b[m_n-1];
            break;
        default:
            interpol=0.0;
            break;
...
//  SAK : generalization to evaluation of both value and derivative at an ordered set of points
...
void spline::eval(const XVector& x, YVector& y, YVector* dydx, double x0, double xScale, int order) const
{
  ...
  // Keep track of search locations so that we can search a smaller range for subsequent points
  int               idx   = 0;
  ...
  for (int i = 0; i < x.size(); ++i) {
    ...
    const double*   geq   = std::lower_bound(start, end, relX);
    idx             = std::max( int(geq - m_x) - 1, idx );
    start           = geq;
